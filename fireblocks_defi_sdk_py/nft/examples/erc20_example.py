...
if __name__ == "__main__":
    ...
    erc20_contract = CustomToken(ropsten_bridge, contract_abi)
    ...
    total_supply = erc20_contract.call_read_function("totalSupply")
    my_balance = erc20_contract.call_read_function("balanceOf", erc20_contract.wallet_address)
    ...
    building_params = {"from": erc20_contract.wallet_address}
    mint_raw_transaction = erc20_contract.call_write_function("mint", erc20_contract.wallet_address, TOKEN_AMOUNT,
                                                              building_params=building_params)
    # We will now submit the transaction to Fireblocks.
    mint_transaction = erc20_contract.submit_transaction(mint_raw_transaction)
    if ropsten_bridge.check_tx_is_completed(mint_transaction['id']) == TRANSACTION_STATUS_COMPLETED:
        print(f"Successfully minted {TOKEN_AMOUNT} to {erc20_contract.wallet_address}")
        transfer_raw_transaction = erc20_contract.call_write_function("transferFrom", erc20_contract.wallet_address,
                                                                      RECEIVER_ADDRESS, TOKEN_AMOUNT,
                                                                      building_params=building_params)
        transfer_transaction = erc20_contract.submit_transaction(transfer_raw_transaction, "Transferring 10 minted.")
        if ropsten_bridge.check_tx_is_completed(transfer_transaction['id']):
            print(f"Successfully transferred {TOKEN_AMOUNT} from {erc20_contract.wallet_address} to {RECEIVER_ADDRESS}")
        else:
