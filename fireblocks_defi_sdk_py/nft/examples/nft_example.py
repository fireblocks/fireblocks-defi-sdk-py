...
if __name__ == "__main__":
    ...
    nft_contract = ERC721(ropsten_bridge)
    ...
    interface_supported = nft_contract.supports_interface()
    if interface_supported:
        print(f"{nft_contract} - supports ERC721 interface.")
    ...
    balance = nft_contract.balance_of()
    print(f"{nft_contract.wallet_address} owns {balance} tokens.")

    if balance:
        # We will now verify if the owner of the token is indeed the wallet, and then transfer it to another address.
        token_owner = nft_contract.owner_of(TOKEN_ID)
        if token_owner == nft_contract.wallet_address:
            receiver_initial_balance = nft_contract.balance_of(RECEIVER_ADDRESS)
            # safe_transfer_from returns a dictionary with a status and id.
            transaction = nft_contract.safe_transfer_from(RECEIVER_ADDRESS, TOKEN_ID)
            ...
            if transaction_result == TRANSACTION_STATUS_COMPLETED:
                ...
                if nft_contract.balance_of(RECEIVER_ADDRESS) == receiver_initial_balance + 1:
